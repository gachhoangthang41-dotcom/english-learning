...
import FlashcardCard from '@/views/components/FlashcardCard';
import { getFlashcardWordId, recordFlashcardReview } from '@/controllers/flashcards';
...
export default function B2TopicFlashcardPage() {
  ...
  const handleMarkComplete = (isLearned: boolean) => {
    void recordFlashcardReview({
      ...
      console.error("Failed to record B2 flashcard review:", error);
    });

    if (isLearned && !completedCards.includes(currentCard.id)) {
      setCompletedCards([...completedCards, currentCard.id]);
      recordLearningProgress("B2", getFlashcardWordId(currentCard.word), currentCard.word, currentCard.meaning);
    }
