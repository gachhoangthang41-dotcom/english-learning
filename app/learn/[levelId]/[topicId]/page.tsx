...
import { ChevronLeft, FileText, Puzzle, Loader2, RefreshCw, CheckCircle, XCircle, SkipBack } from "lucide-react";
...
export default function LearnPage() {
  ...
  const segIndexRef = useRef(segIndex); // Sync ref for event listener
  ...
  const getSegmentText = useCallback((segmentIdx: number): string => {
    const seg = lesson.segments[segmentIdx];
    if (seg && seg.text) {
      return seg.text;
    }
  ...
  }, [lesson.transcript, lesson.segments]);
  ...
  useEffect(() => {
    ...
    const onTimeUpdate = () => {
      // Use ref to get current index inside closure
      const currentIdx = segIndexRef.current;
      const seg = lesson.segments?.[currentIdx];
      ...
      if (v.currentTime >= seg.end - 0.2 && !v.paused) {
        ...
        if (exercise.status === 'idle' || exercise.status === 'loading') {
          if (results[currentIdx]) {
            ...
            setExercise({
              status: 'graded',
              item: results[currentIdx]!.item,
              userAnswer: results[currentIdx]!.userAnswer,
              isCorrect: results[currentIdx]!.isCorrect,
              analysis: results[currentIdx]!.analysis
            });
          } else {
  ...
  }, [lesson.segments, isVideoPausedForQuestion, isLessonComplete, exercise.status, results, fetchQuestion]);
                ...
                {segIndex > 0 && (
                  <button
                    onClick={() => {
                      ...
                      if (videoRef.current && lesson.segments[prevIndex]) {
                        videoRef.current.currentTime = lesson.segments[prevIndex].start;
                      }
