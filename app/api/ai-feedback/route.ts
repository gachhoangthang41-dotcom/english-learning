...
const EXTERNAL_FEEDBACK_API_KEY = process.env.EXTERNAL_FEEDBACK_API_KEY?.trim();
  ...
  try {
    ...
    if (EXTERNAL_FEEDBACK_URL) {
      ...
      try {
        ...
        if (EXTERNAL_FEEDBACK_API_KEY) headers["Authorization"] = `Bearer ${EXTERNAL_FEEDBACK_API_KEY}`;
