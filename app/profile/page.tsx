...
type MeUser = {
  id: string;
  email: string;
  username: string; // username dùng cho đăng nhập/handle
...
export default function ProfilePage() {
  ...
  async function loadMe() {
    ...
    try {
      ...
      // ✅ ưu tiên displayName (biệt danh), fallback về username
            ...
            <aside className="lg:col-span-4">
              <div className="glass-card rounded-2xl border border-white/10 p-5">
                ...
                <div className="mt-5 space-y-2">
                  <MenuItem icon={<Settings className="w-5 h-5" />} label="Chung" active={false} />
                  <MenuItem icon={<User className="w-5 h-5" />} label="Tài khoản" active />
                  <MenuItem icon={<GraduationCap className="w-5 h-5" />} label="Học tập" active={false} />
                  <MenuItem icon={<Volume2 className="w-5 h-5" />} label="Âm thanh" active={false} />
                  <MenuItem icon={<Bell className="w-5 h-5" />} label="Thông báo" active={false} />
                  <MenuItem icon={<Shield className="w-5 h-5" />} label="Bảo mật" active={false} />
                </div>
              ...
              <div className="glass-card rounded-2xl border border-white/10 p-6">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="text-muted mt-1">Xem profile và cập nhật thông tin cơ bản.</p>
                  </div>
                </div>

                <div className={["mt-4 min-h-[20px] text-sm rounded-lg p-3", msg ? messageClass : "hidden"].join(" ")}>
                  {msg?.text || ""}
                </div>

                {/* Info */}
                <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <InfoCard icon={<Mail className="w-4 h-4" />} label="Email" value={loading ? "..." : user?.email || "-"} />

                  {/* ✅ Tên hiển thị = displayName */}
                  <InfoCard
                    icon={<User className="w-4 h-4" />}
                    label="Tên hiển thị"
                    value={loading ? "..." : (user?.displayName || user?.username || "-")}
                  />
                  ...
                  <InfoCard
                    ...
                    value={loading ? "..." : user?.emailVerifiedAt ? "Đã xác thực" : "Chưa xác thực"}
                  />
                </div>
                ...
                <div className="mt-8 grid grid-cols-1 gap-6">
                  <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
                    <h2 className="font-extrabold text-lg">Đổi tên hiển thị (biệt danh)</h2>
                    <p className="text-muted text-sm mt-1">Đổi biệt danh của bạn.</p>
                  ...
                  <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
                    ...
                    <p className="text-muted text-sm mt-1">Để an toàn, cần nhập mật khẩu hiện tại.</p>
                  ...
                  <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
                    ...
                    <p className="text-muted text-sm mt-1">Không thể xem mật khẩu cũ (chỉ đổi).</p>
                ...
                {loading ? <p className="text-sm text-slate-400 mt-6">Đang tải...</p> : null}
              </div>
            </section>
          </div>
        </div>
...
function MenuItem({
  ...
  return (
    ...
    >
      <span className={active ? "text-blue-300" : "text-slate-300"}>{icon}</span>
